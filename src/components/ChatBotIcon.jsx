...
const ChatBotIcon = () => {
  ...
  const portfolioInfo = {
    ...
    contact: {
      email: "atifshahzad.develper@gmail.com",
      github: "github.com/AtifShahzad536",
      linkedin: "linkedin.com/in/atif-shahzad903"
    }
  };
...
#SigmaMind #NoFluff`;
  ...
  return (
    <div className="fixed right-4 z-40" style={{ top: '6rem' }}>
      ...
      {isOpen && (
        <motion.div
          className={`fixed top-4 right-4 z-40 bg-white dark:bg-gray-800 rounded-2xl shadow-2xl overflow-hidden flex flex-col transition-all duration-300 ${isOpen ? 'opacity-100 scale-100' : 'opacity-0 scale-95 pointer-events-none'
            }`}
          style={{
          ...
          <div className="relative flex-1 p-2 overflow-y-auto bg-gradient-to-br from-gray-50 to-gray-100/70 dark:from-gray-900/90 dark:to-gray-800/90 scrollbar-thin scrollbar-thumb-gray-300/70 dark:scrollbar-thumb-gray-600/70 scrollbar-track-transparent" style={{
            ...
            <div className="space-y-1.5 sm:space-y-3">
              {messages.map((msg, index) => (
                ...
                >
                  <motion.div
                    className={`relative max-w-[85%] p-3.5 text-sm sm:text-base transition-all duration-200 ${msg.sender === 'user'
      ...
      <motion.div
        className={`fixed right-4 z-50 rounded-full shadow-xl transition-all duration-300 ${isOpen ? 'scale-0' : 'scale-100'
          }`}
        style={{ top: '6rem' }}
